...
| C.04.093 | issue | minor | not found paths | Not found иногда `nil,nil`, иногда `NewBadRequest("документ не найден")`, иногда plain error. | Ввести `NOT_FOUND` и единый статус для missing entities. |
...
| C.05.095 | issue | minor | logger Seq writer | Единственная явная goroutine - Seq writer; `Close` сигналит done, но ждет через sleep. | Сделать deterministic wait через `sync.WaitGroup` или done ack. |
