...
- Public app settings contract: `organization_short_name` для общего layout должен отдаваться через `GetPublicAppSettings()`, доступный всем авторизованным пользователям. Сами настройки не считаются секретом для чтения, но изменять их может только пользователь с системным правом `admin`.
