...
| A.04.018 | needs_info | High-level scan found no obvious orphan module; fixed document type CRUD is intentionally disabled. | Глубокая проверка dead code не выполнялась в этапе A. | Вернуться на этапе статанализа/code quality. |
