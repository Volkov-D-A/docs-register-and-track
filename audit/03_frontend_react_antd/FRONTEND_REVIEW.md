...
| ISSUE-021 | fixed | Document/settings modals ask for confirmation before discarding touched forms. | D/H |
| ISSUE-022 | fixed | Reference directory tabs extracted from `SettingsPage` into a settings feature component; continue gradual decomposition during feature work. | D/F |
...
| D.01.102 | fixed | none | `SettingsPage.tsx`, `StatisticsPage.tsx`, `AssignmentsPage.tsx`, `DocumentViewModal.tsx` | `SettingsPage` reduced to 1246 lines by extracting reference directories to `features/settings`; other large areas remain candidates for gradual feature-work decomposition. | Keep smoke/build checks after each decomposition. |
...
| D.02.113 | ok | none | document/settings modals | `confirmDiscardFormChanges` checks `form.isFieldsTouched(true)` and asks before closing touched registration/edit/settings forms. | Keep dirty modal smoke in release checklist. |
...
| D.07.149 | ok | none | Vite build | Route-level lazy loading reduced main app chunk to ~34 kB; `npm run build` passes without Vite chunk warning under explicit 1600 kB Wails desktop route-chunk budget. | Performance baseline report generation is release-gated; target OS timing fields must be filled as release evidence. |
...
Статус этапа D: completed with fixed frontend form/error/UX remediation; structural decomposition has started with `ISSUE-022`; keep decomposing large feature areas during normal changes with test/smoke coverage.
