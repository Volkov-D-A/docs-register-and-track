const labels: Record<string, string> = {
  ...
  restoring: 'Восстановление данных и проверка вложений',
...
export const backupStateLabel = (state: string) => labels[state] ?? (state.startsWith('rollback_') ? 'Возврат исходного состояния: ' + state : state);

export const backupKindLabel = (kind: string) => ({ verify: 'Проверка копии', restore: 'Восстановление копии', delete: 'Удаление копии' }[kind] ?? (kind || 'Создание копии'));
