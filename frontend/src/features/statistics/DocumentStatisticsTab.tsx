...
const DocumentStatisticsTab: React.FC = () => {
  ...
  const loadOverview = useCallback(async () => {
    ...
    await Promise.all([
      runLatestStats(async () => {
        const { GetDocumentStatistics } = await import('../../../wailsjs/go/services/StatisticsService');
        return GetDocumentStatistics();
      }, {
        onSuccess: setStats,
      ...
      runLatestFilters(async () => {
        const { GetDocumentFilterOptions } = await import('../../../wailsjs/go/services/StatisticsService');
        return GetDocumentFilterOptions();
      }, {
        onSuccess: (nextFilters) => setFilters(nextFilters || { kinds: [], nomenclature: [], users: [] }),
        onError: (err) => message.error(formatAppError(err)),
  ...
  const loadReport = useCallback(async () => {
    ...
    await runLatestReport(async () => {
      ...
      return GetDocumentReport(range[0].format('YYYY-MM-DD'), range[1].format('YYYY-MM-DD'), groupBy, kind || '', nomenclature || '', user || '');
    }, {
      onSuccess: setReport,
      onError: (err) => message.error(formatAppError(err)),
      onSettled: () => setReportLoading(false),
    });
  }, [groupBy, kind, message, nomenclature, range, runLatestReport, user]);
