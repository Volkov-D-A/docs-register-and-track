...
const SystemStatisticsTab: React.FC = () => {
  ...
  </Row>{stats?.storageRefreshedAt && <Typography.Text type="secondary">Последняя полная сверка MinIO: {new Date(stats.storageRefreshedAt).toLocaleString('ru-RU')}</Typography.Text>}</Spin>;
};
