...
func TestReleaseGateRequiresPostgreSQLIntegration(t *testing.T) {
	...
	integration := strings.Index(content, `run_step "integration-test" "PostgreSQL/S3 integration" make --no-print-directory integration-test`)
	...
	if !(goTest < integration && integration < frontend) {
		t.Fatalf("PostgreSQL/S3 integration must run after Go tests and before frontend checks")
	}
}
