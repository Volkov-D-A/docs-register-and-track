...
func setupAckService(t *testing.T, role string) (
	*AcknowledgmentService, *mocks.AcknowledgmentStore, *mocks.UserStore, *attachmentPrincipalStub, *mocks.IncomingDocStore,
) {
	t.Helper()
...
func TestAcknowledgmentService_GetAllActive(t *testing.T) {
	...
	t.Run("loads recipients only for readable documents", func(t *testing.T) {
		...
		svc := NewAcknowledgmentService(repo, userRepo, auth, access, nil)
...
func TestAcknowledgmentPendingIncludesSubstitutionsAndDeduplicatesRows(t *testing.T) {
	for _, byDocument := range []bool{false, true} {
		name := "all pending"
		if byDocument {
