...
func TestLoadServerBackupConfiguration(t *testing.T) {
	t.Setenv("DOCFLOW_BACKUP_DIRECTORY", "/server-backup")
	t.Setenv("DOCFLOW_SETTINGS_KEY_FILE", "/run/secrets/settings-key")
	t.Setenv("DOCFLOW_BACKUP_MAX_BYTES", "1073741824")
	cfg, err := LoadServer()
	require.NoError(t, err)
	require.Equal(t, "/server-backup", cfg.Backup.Directory)
	require.Equal(t, "/run/secrets/settings-key", cfg.Backup.KeyFile)
	require.EqualValues(t, 1073741824, cfg.Backup.MaxBytes)
	t.Setenv("DOCFLOW_BACKUP_MAX_BYTES", "0")
	_, err = LoadServer()
	require.ErrorContains(t, err, "DOCFLOW_BACKUP_MAX_BYTES")
}
