...
func setupAckService(t *testing.T, role string) (
	*AcknowledgmentService, *mocks.AcknowledgmentStore, *mocks.UserStore, *testPrincipal, *mocks.IncomingDocStore,
	...
	accessSvc := serverservices.NewDocumentAccessService(auth, nil, nil, ackRepo, newRoleMappedDocumentAccessStore(role), &kindBackedDocumentStore{incoming: incomingRepo, outgoing: outgoingRepo})
...
func setupAckServiceNotAuth(t *testing.T) *AcknowledgmentService {
	...
	accessSvc := serverservices.NewDocumentAccessService(auth, nil, nil, ackRepo, newRoleMappedDocumentAccessStore(), &kindBackedDocumentStore{incoming: incomingRepo, outgoing: outgoingRepo})
...
func TestAcknowledgmentService_Create(t *testing.T) {
	...
	t.Run("forbidden executor", func(t *testing.T) {
		...
		result, err := svc.Create(docID.String(), "text", []string{user1.String()})
		require.Error(t, err)
...
func TestAcknowledgmentService_GetAllActive(t *testing.T) {
	...
	t.Run("loads recipients only for readable documents", func(t *testing.T) {
		...
		access := serverservices.NewDocumentAccessService(auth, nil, nil, repo, accessStore, &kindBackedDocumentStore{incoming: incomingRepo})
...
func TestAcknowledgmentServiceMarkViewedPassesJournalEffectToAtomicStore(t *testing.T) {
	ackID := uuid.New()
	svc, repo, _, auth, _ := setupAckService(t, "executor")
	userID, err := uuid.Parse(auth.GetCurrentUserID())
	require.NoError(t, err)
	atomicRepo := &atomicAcknowledgmentStore{AcknowledgmentStore: repo}
	svc.repo = atomicRepo
...
func TestAcknowledgmentServiceMarkConfirmedPassesUserEventsToAtomicStore(t *testing.T) {
	ackID, documentID, creatorID := uuid.New(), uuid.New(), uuid.New()
