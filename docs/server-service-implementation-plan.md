...
операции документов реализованы и проверены через server API.
...
| 5. Business API | В работе | Пользователи, профиль, справочники, настройки и read-only документы перенесены; следующий срез — команды документов |
...
Контрольный commit до текущего рабочего среза — `0a22a07`. При возобновлении
...
В текущем рабочем срезе завершены **read-only списки и карточки документов** для
входящих, исходящих, обращений граждан и приказов. `DocumentQueryService`
...
Следующий рекомендуемый срез — команды регистрации и изменения документов с
idempotency.

После него рекомендуемый порядок:

1. Команды регистрации/изменения документов с idempotency.
2. Поручения, ознакомления, связи, journal, dashboard и statistics.
