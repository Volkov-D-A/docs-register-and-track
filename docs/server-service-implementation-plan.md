...
операции документов, а также команды регистрации, изменения и создания
административных черновиков реализованы через server API.
...
| 5. Business API | В работе | Пользователи, профиль, справочники, настройки и документы перенесены; следующий срез — поручения |
...
В текущем рабочем срезе команды регистрации, изменения и создания
...
Следующий рекомендуемый срез — поручения, соисполнители и связанные замещения.

После него рекомендуемый порядок:

1. Поручения, соисполнители и связанные замещения.
2. Ознакомления, user events, связи, journal, dashboard и statistics.
3. Вложения через streaming API, затем закрытие прямого PostgreSQL/MinIO.
