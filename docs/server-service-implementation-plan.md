...
Дата последнего обновления: 1 сентября 2026 года
...
- списки и карточки всех поддерживаемых видов документов читаются через server
  API; access scope вычисляется только из bearer principal на сервере и не может
  быть передан клиентом;
...
- регистрация, изменение и создание административных черновиков всех четырёх
  видов документов выполняются через server API с обязательным
  `Idempotency-Key`; одинаковый ключ с изменённым payload возвращает conflict,
  а документ, номер, journal/outbox и результат команды фиксируются атомарно.
- создание, чтение, изменение, удаление и смена статуса поручений, включая
  соисполнителей, повторяющиеся серии и историю итераций, выполняются через
  server API; bearer principal и активные замещения определяют доступ на
  сервере, а journal/user-event effects сохраняются через transactional outbox.
- создание, чтение, просмотр, подтверждение и удаление ознакомлений, а также
  отметка строк листа ознакомления приказа, чтение и отметки персональных
...
В одном из завершённых срезов реализовано **основное управление пользователями
через server API**:
...
Перед текущим рабочим срезом завершены **системные настройки**: административные
...
Перед текущим рабочим срезом завершены **read-only списки и карточки
документов** для входящих, исходящих, обращений граждан и приказов. `DocumentQueryService`
переключён на typed server client, а server query engine получает неизменяемый
request-local principal. Фильтр `AccessScope` не сериализуется и попытка передать
его в API отклоняется; сервер вычисляет область видимости по правам,
подразделению, поручениям, ознакомлениям и активным замещениям.
...
Сервер создаёт request-local principal из bearer-сессии и самостоятельно
вычисляет область доступа с учётом активных замещений; переданный клиентом
внутренний assignment scope отклоняется. Unit, HTTP/client, frontend и полный
PostgreSQL integration-прогон проходят.

После них перенесены ознакомления и user events: создание, списки, просмотр,
подтверждение и удаление ознакомлений, отметка строк листа ознакомления приказа,
а также список, счётчик и отметки чтения персональных событий работают через
typed server client. Desktop-сервисы больше не используют свои repository
...
### Завершённый срез: управление пользователями
