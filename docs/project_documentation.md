...
- `document_resolutions` — общая резолюция документа: текст, автор и исполнители резолюции

Для входящего письма собственные регистрационные номер и дата остаются в `incoming_document_details`, а реквизиты корреспондента и резолюция вынесены в общие таблицы. Это позволяет переиспользовать эти наборы полей в будущих видах документов без расширения типоспецифичных таблиц.
